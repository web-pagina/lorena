...
async function loadObras() {
  try {
    ...
    return data.map((r, idx) => {
      ...
      const categoryRaw = r.category || r.tipo_original || (r._original_row && r._original_row['Tipo']) || "Obra pictórica";
      const category = categoryRaw; // dejamos el original por si lo queremos mostrar, pero usamos normalización al filtrar
      return {
        _idx: idx,
...
function normalize(str) {
  if (!str && str !== 0) return "";
  // pasar a string, quitar tildes, pasar a minúsculas y trim
  return String(str)
